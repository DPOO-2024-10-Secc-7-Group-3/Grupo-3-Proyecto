...
public class MainOperador extends Main
{
	public MainOperador(Usuario user)
	{
		super(user);
	}
		...
		try
		{
			...
			while (working)
			{
				...
				else if(input.equals("6"))
				{
					...
					BufferedReader read1 = new BufferedReader(new InputStreamReader(System.in));
					String pieza = read1.readLine();
					
					Pieza nPieza = Pieza.piezas.get(pieza);
					Operador operador = (Operador) user;
					operador.getSubastas().add((Subasta)nPieza.getDisponibilidad());
					user = operador;
				}
